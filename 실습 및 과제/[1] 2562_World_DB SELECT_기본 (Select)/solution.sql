...
select code, name, gnp, gnpold, (GNP - GNPOld) AS "gnp변동량" from country where gnp-gnpold IS NOT NULL ;
...
# mysql에서 문자열을 연결할 경우는 concat 함수를 사용한다.
select concat(name,'은 ', region,'에 속하며 인구는 ', population,'명이다.') "정보" from country where continent='Asia';

# 7. country에서 독립 년도에 대한 기록이 없고 인구가 10000 이상인 국가의 정보를 아래와 같이 인구의 오름차순으로 출력하시오.
...
select code, name from country where substring(code, 1, 1) in ('a','e','i','o','u') order by name limit 2, 3;

# 14. country에서 name을 맨 앞과 맨 뒤에 2글자를 제외하고 나머지는 *로 처리해서 출력하시오. *****
SELECT name, concat(rpad(left(name, 2), length(name) - 2, '*'), right(name, 2)) 'guess' FROM country;

# 15. country에서 region을 중복 없이 가져오는데 공백을 _로 대체하시오. ***
select distinct replace(region, ' ', '_') "지역들" from country order by length("지역들") desc;

# 16. country에서 인구가 1억 이상인 국가들의 1인당 점유면적(surfacearea/population)을 반올림해서 소숫점 3자리로 표현하시오. 1인당 점유 면적의 오름차순으로 정렬한다.
select name, surfacearea, population, round(surfacearea/population, 3) as '1인당 점유면적' from country where population >= 100000000 order by '1인당 점유면적';